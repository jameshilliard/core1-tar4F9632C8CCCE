		   ...
		   char *optval, int optlen)
{
	...
	switch (optname) {
#if CONFIG_DANUBE
            #if 0 //060623:henryhsu Modify for RTVBR
            case 0x4202006C:
            {
                struct atm_trafprm_temp {
...
//  #endif
                ...
                struct atm_qos_temp {
                    struct atm_trafprm_temp txtp;                    /* parameters in TX direction */
                    struct atm_trafprm_temp rxtp __ATM_API_ALIGN;    /* parameters in RX direction */
                    unsigned char aal __ATM_API_ALIGN;
                };

                struct atm_qos_temp qos_temp;

                struct atm_qos qos;

                if ( sizeof(qos_temp) != 0x6C )
                {
                    printk("sizeof qos_temp = %d, expect value is 108\n", sizeof(qos_temp));
                    break;
                }

                if (copy_from_user(&qos_temp,optval,sizeof(qos_temp)))
                    return -EFAULT;

                qos.aal = qos_temp.aal;

                qos.txtp.traffic_class   = qos_temp.txtp.traffic_class;
                qos.txtp.max_pcr         = qos_temp.txtp.max_pcr;
                qos.txtp.pcr             = qos_temp.txtp.pcr;
                qos.txtp.min_pcr         = qos_temp.txtp.min_pcr;
                qos.txtp.max_cdv         = qos_temp.txtp.max_cdv;
                qos.txtp.max_sdu         = qos_temp.txtp.max_sdu;
                qos.txtp.icr             = qos_temp.txtp.icr;
                qos.txtp.tbe             = qos_temp.txtp.tbe;
                qos.txtp.frtt            = qos_temp.txtp.frtt;
                qos.txtp.rif             = qos_temp.txtp.rif;
                qos.txtp.rdf             = qos_temp.txtp.rdf;
                qos.txtp.nrm_pres        = qos_temp.txtp.nrm_pres;
                qos.txtp.trm_pres        = qos_temp.txtp.trm_pres;
                qos.txtp.adtf_pres       = qos_temp.txtp.adtf_pres;
                qos.txtp.cdf_pres        = qos_temp.txtp.cdf_pres;
                qos.txtp.nrm             = qos_temp.txtp.nrm;
                qos.txtp.trm             = qos_temp.txtp.trm;
                qos.txtp.adtf            = qos_temp.txtp.adtf;
                qos.txtp.cdf             = qos_temp.txtp.cdf;
                qos.txtp.spare           = qos_temp.txtp.spare;

                qos.rxtp.traffic_class   = qos_temp.rxtp.traffic_class;
                qos.rxtp.max_pcr         = qos_temp.rxtp.max_pcr;
                qos.rxtp.pcr             = qos_temp.rxtp.pcr;
                qos.rxtp.min_pcr         = qos_temp.rxtp.min_pcr;
                qos.rxtp.max_cdv         = qos_temp.rxtp.max_cdv;
                qos.rxtp.max_sdu         = qos_temp.rxtp.max_sdu;
                qos.rxtp.icr             = qos_temp.rxtp.icr;
                qos.rxtp.tbe             = qos_temp.rxtp.tbe;
                qos.rxtp.frtt            = qos_temp.rxtp.frtt;
                qos.rxtp.rif             = qos_temp.rxtp.rif;
                qos.rxtp.rdf             = qos_temp.rxtp.rdf;
                qos.rxtp.nrm_pres        = qos_temp.rxtp.nrm_pres;
                qos.rxtp.trm_pres        = qos_temp.rxtp.trm_pres;
                qos.rxtp.adtf_pres       = qos_temp.rxtp.adtf_pres;
                qos.rxtp.cdf_pres        = qos_temp.rxtp.cdf_pres;
                qos.rxtp.nrm             = qos_temp.rxtp.nrm;
                qos.rxtp.trm             = qos_temp.rxtp.trm;
                qos.rxtp.adtf            = qos_temp.rxtp.adtf;
                qos.rxtp.cdf             = qos_temp.rxtp.cdf;
                qos.rxtp.spare           = qos_temp.rxtp.spare;
