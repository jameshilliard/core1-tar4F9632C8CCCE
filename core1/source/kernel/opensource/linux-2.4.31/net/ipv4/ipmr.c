...
#if defined(CONFIG_DANUBE_PPA) || defined(CONFIG_DANUBE_PPA_MODULE)
...
static int ipmr_mfc_delete(struct mfcctl *mfc)
{
	...
	for (cp=&mfc_cache_array[line]; (c=*cp) != NULL; cp = &c->next) {
		...
		{
			PPA_MC_GROUP ppa_mc_entry, ppa_mc_entry_cmp;
			
			int n, idx;
			
			memset(&ppa_mc_entry, 0, sizeof(ppa_mc_entry));
			memset(&ppa_mc_entry_cmp, 0, sizeof(ppa_mc_entry));

			ppa_hook_mc_group_get_fn(c->mfc_mcastgrp, &ppa_mc_entry_cmp, NULL);


			ppa_mc_entry.if_mask = ppa_mc_entry_cmp.if_mask;
			for(n=c->mfc_un.res.minvif;n<c->mfc_un.res.maxvif;n++)
			{
				if(VIF_EXISTS(n) && c->mfc_un.res.ttls[n] < 255)
				{		
					int i;
			...
			ppa_hook_mc_group_update_fn(&ppa_mc_entry, NULL);
		}	
#endif
...
#if 0
#if defined(CONFIG_DANUBE_PPA) || defined(CONFIG_DANUBE_PPA_MODULE)
    if ( ppa_hook_mc_session_del_fn != NULL )
    {
        ppa_hook_mc_session_del_fn(mfc->mfcc_mcastgrp.s_addr, 0);
    }
#endif
#endif
...
#if 0
#if defined(CONFIG_DANUBE_PPA) || defined(CONFIG_DANUBE_PPA_MODULE)
    if ( ppa_hook_mc_session_add_fn != NULL )
    {
        PPA_MC_SESSION mc_session;

        mc_session.ip_mc_group = skb->nh.iph->daddr;
        mc_session.num_ifs = 1;

        memcpy(mc_session.src_mac, skb->mac.ethernet->h_source, PPA_ETH_ALEN);
        mc_session.replace_src_mac = 0;

        mc_session.array_mem_ifs[0] = skb->dev->name;

        ppa_hook_mc_session_add_fn((PPA_BUF *)skb, &mc_session, 0);
    }
#endif
#endif	
