...
static void death_by_timeout(unsigned long ul_conntrack)
{
	...
	{
		int result;
		result = ppa_hook_inactivity_status_fn((PPA_U_SESSION *)ct); 
#if 0
		printk("ct:%x death_by_timeout:%d\n",ct ,result);
		DUMP_PPA_TUPLE(&(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple));
		DUMP_PPA_TUPLE(&(ct->tuplehash[IP_CT_DIR_REPLY].tuple));
#endif		
		if ( result == IFX_PPA_HIT)
		{
	       	 ip_ct_refresh(ct, 60 * HZ); //to check again after default seconds
			return;
		}
...
#if 0
	if((*pskb)->pkt_type == 1 || (*pskb)->pkt_type == 2 ||(*pskb)->nh.iph->protocol ==1)
		;
	else
	{
		printk("ip_conntrack_in, add session:%x\n", ct);
		DUMP_PPA_TUPLE(&(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple));
		DUMP_PPA_TUPLE(&(ct->tuplehash[IP_CT_DIR_REPLY].tuple));
	}
#endif	
...
void ip_ct_refresh(struct ip_conntrack *ct, unsigned long extra_jiffies)
{
	...
	if (!is_confirmed(ct))
    	...
    	{
    		//printk("ip_ct_refresh:set to zero\n");
