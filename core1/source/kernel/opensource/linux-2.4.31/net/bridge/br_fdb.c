...
#if defined(CONFIG_DANUBE_PPA) || defined(CONFIG_DANUBE_PPA_MODULE)
            if ( ppa_hook_bridge_entry_hit_time_fn && !f->is_local )
            {
...
void br_fdb_put(struct net_bridge_fdb_entry *ent)
{
	if (atomic_dec_and_test(&ent->use_count))
	{
        ...
        {
            ppa_hook_bridge_entry_delete_fn(ent->addr.addr, 0);
        }
#endif
		kfree(ent);
	}	
...
#if defined(CONFIG_DANUBE_PPA) || defined(CONFIG_DANUBE_PPA_MODULE)
	if ( ppa_hook_bridge_entry_add_fn && !is_local && source->dev )
	{
        ppa_hook_bridge_entry_add_fn(addr, source->dev, 0);
	}
#endif
