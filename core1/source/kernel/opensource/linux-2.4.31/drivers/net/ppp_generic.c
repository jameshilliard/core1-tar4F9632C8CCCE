...
#if (defined(CONFIG_DANUBE_PPA) || defined(CONFIG_DANUBE_PPA_MODULE)) && defined(CONFIG_PPPOE)
  #include <net/sock.h>
#endif
...
int32_t pppoe_get_pppoe_addr(struct net_device *dev, struct pppoe_addr *pa)
{
    ...
    spin_lock_bh(&pch->downl);
    if ( pch->chan && pch->chan->private )
    {
        sock = (struct sock *)pch->chan->private;
...
__u16 ppa_get_pkt_pppoe_session_id(struct sk_buff *skb)
{
    ...
    if ( !pch->chan || !pch->chan->private || !((struct sock *)pch->chan->private)->protinfo.pppox )
        return 0;

    return ((struct sock *)pch->chan->private)->protinfo.pppox->proto.pppoe.pa.sid;
}

int32_t ppa_pppoe_get_eth_netif(struct net_device *netif, char pppoe_eth_ifname[IFNAMSIZ])
{
    struct channel *pch;
    struct pppoe_addr *pppoe_addr;
    int32_t ret = -1;

    if ( !netif || !pppoe_eth_ifname )
        return -1;
    ...
    if ( !netif->priv )
        return -1;   //  no PPP info

    if ( !((struct ppp *)netif->priv)->channels.next )
        return -1;

    pch = list_entry(((struct ppp *)netif->priv)->channels.next, struct channel, clist);

    spin_lock_bh(&pch->downl);

    if ( pch->chan && pch->chan->private && ((struct sock *)pch->chan->private)->protinfo.pppox )
    {
        strcpy(pppoe_eth_ifname, ((struct sock *)pch->chan->private)->protinfo.pppox->pppoe_pa.dev);
        ret = 0;
    }
...
EXPORT_SYMBOL(pppoe_get_pppoe_addr);
