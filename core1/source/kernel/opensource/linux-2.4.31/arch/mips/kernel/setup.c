...
#include "../arch/mips/infineon/danube/mps/mps_device.h"
#endif
//prochao-, ---------
#ifdef CONFIG_AMAZON
...
extern	u32	*danube_cp1_base, *mps_tpe_cpu1_entry_base;
#endif
...
//prochao+,
#if	defined(CONFIG_DANUBE)||defined(CONFIG_BOARD_TWINPASS_E)		//for dual-core
extern void prom1_init(int, char **, char **, int *);
#endif
//prochao-
...
{
#if	defined(CONFIG_DANUBE)||defined(CONFIG_BOARD_TWINPASS_E)		//for dual-core
	unsigned long	waitloop = 1;			//prochao,
#endif
	mps_mbx_reg		*MPS_MBX_Pointer;	//prochao

	/* Determine which MIPS variant we are running on. */
	cpu_probe();

//prochao, 7-26-2006, depends on the CPU-ID (CPU0 or CPU1)
// Noted that the ebase_cpu_num field is read inside the previou cpu_probe() function
//CPU1 does not need to parse the prom input
	if ((current_cpu_data.ebase_cpu_num & 0x03FF) == 0)
	{	// CPU0, as usual (original processing)
		prom_init(argc, argv, envp, prom_vec);
		//prochao+, kick-out the CPU1 to take over and run this image <<=================
#if	defined(CONFIG_DANUBE)||defined(CONFIG_BOARD_TWINPASS_E)		//for dual-core
		if (danube_cp1_base != (u32*) NULL)
		{
			MPS_MBX_Pointer = (mps_mbx_reg *)(0xA0000000 | 0x1F200000);	//DANUBE_MPS_SRAM;	//(KSEG1 | ...)
			/* avoid to overwrite CPU boot registers */
			memset((void*) MPS_MBX_Pointer, 0, sizeof(mps_mbx_reg) - 2*sizeof(mps_boot_cfg_reg));
			//ready to kick out the CPU1 at the given address: danube_cp1_base
#if 0	//move this inside the MPS driver, till the necessary initializations of MPS are done
			...
			*DANUBE_RCU_RST_REQ |= (DANUBE_RCU_RST_REQ_CPU1);
			...
			MPS_MBX_Pointer->MBX_CPU1_BOOT_CFG.MPS_BOOT_SIZE = 0;	// ignored, according to the datasheet
			MPS_MBX_Pointer->MBX_CPU1_BOOT_CFG.MPS_BOOT_RVEC = (u32) danube_cp1_base;
			// follow the code fragment inside the ifx_mps_release()
			*DANUBE_RCU_RST_REQ |= 0x20000000;
			*DANUBE_RCU_RST_REQ &= (~8);
			wmb();
#else
			mps_tpe_cpu1_entry_base = danube_cp1_base;	//keep it for later triggering
#endif	//
		}
#endif	//prochao-, ----------------------------
	}
	...
	{	// this must be for the CPU1 and should be running in the memory above 2nd 16MB area
		// this is created equivalent to the prom_init() with some differences
		prom1_init(argc, argv, envp, prom_vec);
	}
#endif
	//
	cpu_report();
	 ...
	 */
	load_mmu();
	//prochao+, keeps CPU0 staying here for help debugging the CPU1
#if	0	//defined(CONFIG_DANUBE)||defined(CONFIG_BOARD_TWINPASS_E)	//for dual-core, realtime running
	waitloop = 0x38000000;	//magic# for CPU1 be ready to go!
	if ((current_cpu_data.ebase_cpu_num & 0x03FF) == 0)
...
void __init setup_arch(char **cmdline_p)
{
	...
	case MACH_GROUP_AMAZON:
		...
		break;
#endif
// prochao, 06-26-2006, adds for TwinPass support
#if	defined(CONFIG_DANUBE)||defined(CONFIG_TWINPASS)
