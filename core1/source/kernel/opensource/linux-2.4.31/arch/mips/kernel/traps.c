...
void *set_except_vector(int n, void *addr)
{
	...
	if (n == 0 && cpu_has_divec) {
		*(volatile u32 *)((KSEG0 | OFFSET_ABOVE_16MB)+0x200) = 0x08000000 | (0x03ffffff & (handler >> 2));
		flush_icache_range((KSEG0 | OFFSET_ABOVE_16MB)+0x200, (KSEG0 | OFFSET_ABOVE_16MB) + 0x204);
	}
...
void __init trap_init(void)
{
	...
	memcpy((void *)((KSEG0 | OFFSET_ABOVE_16MB) + 0x80), &except_vec1_generic, 0x80);
	 ...
	 */
	if (cpu_has_ejtag)
		memcpy((void *)((KSEG0 | OFFSET_ABOVE_16MB) + 0x300), &except_vec_ejtag_debug, 0x80);
	 ...
	 */
	if (cpu_has_divec)
		memcpy((void *)((KSEG0 | OFFSET_ABOVE_16MB) + 0x200), &except_vec4, 8);
	...
	if (cpu_has_vce)
		memcpy((void *)((KSEG0 | OFFSET_ABOVE_16MB) + 0x180), &except_vec3_r4000, 0x80);
	else if (cpu_has_4kex)
		memcpy((void *)((KSEG0 | OFFSET_ABOVE_16MB) + 0x180), &except_vec3_generic, 0x80);
	else
		memcpy((void *)((KSEG0 | OFFSET_ABOVE_16MB) + 0x080), &except_vec3_generic, 0x80);
	...
	flush_icache_range((KSEG0 | OFFSET_ABOVE_16MB), (KSEG0 | OFFSET_ABOVE_16MB) + 0x400);

	per_cpu_trap_init();
	//prochao, 7-25-2006, rebase the interrupt vectors entry
#if	defined(CONFIG_DANUBE_CORE1)||defined(CONFIG_TWINPASS_E_CORE1)		//for the dual-core
	// Note: at this moment, the interrupts should be disabled!
	i = read_c0_status() | ST0_BEV;	// set the BEV bit
	write_c0_ebase(KSEG0 | OFFSET_ABOVE_16MB);
	i &= ~ST0_BEV;						// reset the BEV bit
	write_c0_status( i);
#endif
}
