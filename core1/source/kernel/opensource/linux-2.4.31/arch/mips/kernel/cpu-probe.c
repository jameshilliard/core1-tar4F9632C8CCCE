...
//prochao-,==================
	...
	cp0_status = read_c0_status() | ST0_BEV;	// set the BEV bit
	write_c0_ebase(KSEG0 | OFFSET_ABOVE_16MB);
	cp0_status &= ~ST0_BEV;						// reset the BEV bit
	write_c0_status( cp0_status);
	//readout the EBase and log
	c->ebase_cpu_num = read_c0_ebase();
#endif
}
