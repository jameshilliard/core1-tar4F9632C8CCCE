...
{
	unsigned long start = (KSEG0 | OFFSET_ABOVE_16MB);
					...
					unsigned long page)
{
	 ...
	 * to work correctly.
	 */
	page = ((KSEG0 | OFFSET_ABOVE_16MB) + (page & (dcache_size - 1)));
	if (cpu_has_dc_aliases || (exec && !cpu_has_ic_fills_f_dc))
...
static inline void rm7k_erratum31(void)
{
	...
	for (addr = (KSEG0 | OFFSET_ABOVE_16MB); addr <= (KSEG0 | OFFSET_ABOVE_16MB) + 4096; addr += ic_lsize) {
		__asm__ __volatile__ (
			".set noreorder\n\t"
			".set mips3\n\t"
			"cache\t%1, 0(%0)\n\t"
...
void __init ld_mmu_r4xx0(void)
{
	...
	memcpy((void *)((KSEG0 | OFFSET_ABOVE_16MB) + 0x100), &except_vec2_generic, 0x80);
	memcpy((void *)((KSEG1 | OFFSET_ABOVE_16MB) + 0x100), &except_vec2_generic, 0x80);
