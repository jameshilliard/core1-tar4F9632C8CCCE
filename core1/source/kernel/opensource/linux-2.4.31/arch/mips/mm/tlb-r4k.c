...
void local_flush_tlb_all(void)
{
	...
	while (entry < current_cpu_data.tlbsize) {
		write_c0_entryhi((KSEG0 | OFFSET_ABOVE_16MB) + entry*0x2000);
	...
	if (cpu_context(cpu, mm) != 0) {
		...
		if (size <= current_cpu_data.tlbsize/2) {
			...
			while (start < end) {
				...
				write_c0_entryhi((KSEG0 | OFFSET_ABOVE_16MB) + idx*0x2000);
				mtc0_tlbw_hazard();
				tlb_write_indexed();
			}
			tlbw_use_hazard();
...
void __init r4k_tlb_init(void)
{
	...
	if (cpu_has_4kex && cpu_has_4ktlb) {
		if (current_cpu_data.cputype == CPU_NEVADA)
			memcpy((void *)(KSEG0 | OFFSET_ABOVE_16MB), &except_vec0_nevada, 0x80);
		else if (current_cpu_data.cputype == CPU_R4600)
			memcpy((void *)(KSEG0 | OFFSET_ABOVE_16MB), &except_vec0_r4600, 0x80);
		else
			memcpy((void *)(KSEG0 | OFFSET_ABOVE_16MB), &except_vec0_r4000, 0x80);
		flush_icache_range((KSEG0 | OFFSET_ABOVE_16MB), (KSEG0 | OFFSET_ABOVE_16MB) + 0x80);
	}
}
