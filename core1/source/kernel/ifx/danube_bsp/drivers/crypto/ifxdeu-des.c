 ...
 *
 */
static int des3_ede_setkey(void *ctx, const u8 *key, unsigned int keylen, u32 *flags)
{
	volatile struct des_t	*des = (struct des_t *) DES_3DES_START;
	unsigned long			flag;
...
int __init ifxdeu_init_des(void)
{
	...
	{
		des_alg.cra_u.cipher.cia_max_nbytes = (size_t) - 1;
		...
		des3_ede_alg.cra_u.cipher.cia_max_nbytes = (size_t) - 1;
	...
	if(ret < 0) crypto_unregister_alg(&des_alg);
out:
	return ret;
}
