...
extern struct semaphore mei_sema;
extern int showtime;
...
static int danube_mei_mib_shutdown = 0;
	...
	case GET_ADSL_LINE_RATE:
		...
		if(IS_FLAG_SET((&(pts.adslLineRateInfo_pt->flags)), LINE_RATE_ATTNDRUS_FLAG)){
			...
			}else
			{
				...
				hdlc_cmd[1]=0x24;
				up(&mei_sema);
				if (ifx_mei_hdlc_send((char *)&hdlc_cmd[0],4)!= -EBUSY)
				{
	...
	case GET_ADSL_FAREND_STATS:
		...
		if (adsl_mode>8 || adsl_mode_extend > 0)
		{
			...
			{								
				...
				if (ifx_mei_hdlc_send((char *)&hdlc_cmd[0],2)!= -EBUSY)
				{
					set_current_state(TASK_INTERRUPTIBLE);
					schedule_timeout(1);		
					hdlc_rx_len=0;			
...
// 603221:tc.chen end
	case GET_ADSL_ATUC_SUBCARRIER_STATS:
		...
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_HLINSC)){
			FAREND_HLINSC_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group PLAM Address 35 Index 0");
#endif
				pts.adslATUCSubcarrierInfo_pt->HLINSCds = 0;	
			}
			else{
				pts.adslATUCSubcarrierInfo_pt->HLINSCds = RxMessage[4];
				
			}
		}
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_HLINPS)){
			int index=0,size=12;
			...
			for (index=0;index<1024;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					memcpy(&pts.adslATUCSubcarrierInfo_pt->HLINpsds[index],&RxMessage[4],size*2);
#if 0
					int msg_idx;
		...
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_HLOGMT)){
			FAREND_HLOGMT_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group PLAM Address 35 Index 0");
#endif
				pts.adslATUCSubcarrierInfo_pt->HLOGMTds = 0;	
			}
			else{
				pts.adslATUCSubcarrierInfo_pt->HLOGMTds = RxMessage[4];
				
			}
		}
		
		/////////////////////////////////////////////////////////////////////////
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_HLOGPS)){
			...
			for (index=0;index<256;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					{
						memcpy(&pts.adslATUCSubcarrierInfo_pt->HLOGpsds[index],&RxMessage[4],size*2);
					}else
					{
						int msg_idx=0;
						for (msg_idx=0;msg_idx<size;msg_idx++)
						{
							pts.adslATUCSubcarrierInfo_pt->HLOGpsds[(index+msg_idx)*2+1] = RxMessage[4+msg_idx];
							//printk("index:%d ,cmv_result: %04X\n",index+msg_idx,RxMessage[4+msg_idx]);
						}
					}	
				}
			...
			if (adsl_mode >= 0x4000 || adsl_mode_extend >0)//adsl2+ mode
			{
				pts.adslATUCSubcarrierInfo_pt->HLOGpsds[0] = pts.adslATUCSubcarrierInfo_pt->HLOGpsds[1];
				for (index=1;index<256;index++)
				{
					pts.adslATUCSubcarrierInfo_pt->HLOGpsds[index*2]   = (pts.adslATUCSubcarrierInfo_pt->HLOGpsds[(index)*2-1] +  pts.adslATUCSubcarrierInfo_pt->HLOGpsds[(index)*2+1] +1) >>1;
				}
			}
		}
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_QLNMT)){
			FAREND_QLNMT_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group PLAM Address 35 Index 0");
#endif
				pts.adslATUCSubcarrierInfo_pt->QLNMTds = 0;	
			}
			else{
				pts.adslATUCSubcarrierInfo_pt->QLNMTds = RxMessage[4];					
			}
		}
		
		/////////////////////////////////////////////////////////////////////////
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_QLNPS)){
			int index=0,size=12;
			//printk("FAREND_QLNPS\n");
			for (index=0;index<128;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
						...
						if (adsl_mode < 0x4000 && adsl_mode_extend==0)//adsl2 mode
						{
							pts.adslATUCSubcarrierInfo_pt->QLNpsds[(index+msg_idx)*2] = (u16)(RxMessage[4+msg_idx]&0xFF);
							pts.adslATUCSubcarrierInfo_pt->QLNpsds[(index+msg_idx)*2+1] = (u16)((RxMessage[4+msg_idx]>>8)&0xFF);
						}else
						{
							pts.adslATUCSubcarrierInfo_pt->QLNpsds[(index+msg_idx)*4+1] = (u16)(RxMessage[4+msg_idx]&0xFF);
							pts.adslATUCSubcarrierInfo_pt->QLNpsds[(index+msg_idx)*4+3] = (u16)((RxMessage[4+msg_idx]>>8)&0xFF);
							//printk("index:%d ,cmv_result: %04X\n",index+msg_idx,RxMessage[4+msg_idx]);
						}
					}
			...
			if (adsl_mode >= 0x4000 || adsl_mode_extend >0)//adsl2+ mode
			{
				pts.adslATUCSubcarrierInfo_pt->QLNpsds[0] = pts.adslATUCSubcarrierInfo_pt->QLNpsds[1];
				for (index=1;index<256;index++)
				{
					pts.adslATUCSubcarrierInfo_pt->QLNpsds[index*2]   = (pts.adslATUCSubcarrierInfo_pt->QLNpsds[(index)*2-1] +  pts.adslATUCSubcarrierInfo_pt->QLNpsds[(index)*2+1]) >>1;
				}
			}
		}
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_SNRMT)){
			FAREND_SNRMT_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group PLAM Address 35 Index 0");
#endif
				pts.adslATUCSubcarrierInfo_pt->SNRMTds = 0;	
			}
			else{
				pts.adslATUCSubcarrierInfo_pt->SNRMTds = RxMessage[4];					
			}
		}
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_SNRPS)){
			int index=0,size=12;
			//printk("FAREND_SNRPS\n");
			for (index=0;index<512;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					//memcpy(&pts.adslATUCSubcarrierInfo_pt->SNRpsds[index],&RxMessage[4],size*2);
					int msg_idx=0;
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
						pts.adslATUCSubcarrierInfo_pt->SNRpsds[index+msg_idx] = (u16)(RxMessage[4+msg_idx]&0xFF);
						//printk("index:%d ,cmv_result: %04X\n",index+msg_idx,RxMessage[4+msg_idx]);
					}
		...
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_BITPS)){
			int index=0,size=12;
			//printk("FAREND_BITPS\n");
			for (index=0;index<256;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
						pts.adslATUCSubcarrierInfo_pt->BITpsds[(index+msg_idx)*2] = (u16)(RxMessage[4+msg_idx]&0xFF);
						pts.adslATUCSubcarrierInfo_pt->BITpsds[(index+msg_idx)*2+1] = (u16)((RxMessage[4+msg_idx]>>8)&0xFF);
		...
		if(IS_FLAG_SET((&(pts.adslATUCSubcarrierInfo_pt->flags)),  FAREND_GAINPS)){
			int index=0,size=12;
			...
			for (index=0;index<512;index+=size)
			{
				...
				/*
					...
					*/
					memcpy(&pts.adslATUCSubcarrierInfo_pt->GAINpsds[index],&RxMessage[4],size*2);	
#if 0
					int msg_idx=0;
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
	...
	case GET_ADSL_ATUR_SUBCARRIER_STATS:
		...
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_HLINSC)){
			NEAREND_HLINSC_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group INFO Address 71 Index 2");
#endif
				pts.adslATURSubcarrierInfo_pt->HLINSCus = 0;	
			}
			else{
				pts.adslATURSubcarrierInfo_pt->HLINSCus = RxMessage[4];
				
			}
		}
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_HLINPS)){
			int index=0,size=12;
			...
			for (index=0;index<128;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					memcpy(&pts.adslATURSubcarrierInfo_pt->HLINpsus[index],&RxMessage[4],size*2);
#if 0
					int msg_idx;
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
		...
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_HLOGMT)){
			NEAREND_HLOGMT_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group INFO Address 80 Index 0");
#endif
				pts.adslATURSubcarrierInfo_pt->HLOGMTus = 0;	
			}
			else{
				pts.adslATURSubcarrierInfo_pt->HLOGMTus = RxMessage[4];
				
			}
		}
		
		/////////////////////////////////////////////////////////////////////////
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_HLOGPS)){
			int index=0,size=12;
			...
			for (index=0;index<64;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					{
#endif
					memcpy(&pts.adslATURSubcarrierInfo_pt->HLOGpsus[index],&RxMessage[4],size*2);
#if 0
					}else
					{
						int msg_idx=0;
						for (msg_idx=0;msg_idx<size;msg_idx++)
						{
							//pts.adslATURSubcarrierInfo_pt->HLOGpsus[(index+msg_idx)*2+1] = RxMessage[4+msg_idx];
							pts.adslATURSubcarrierInfo_pt->HLOGpsus[(index+msg_idx)] = RxMessage[4+msg_idx];
						}
					}						
#endif
				}
			...
			{
				pts.adslATURSubcarrierInfo_pt->HLOGpsus[0] = pts.adslATURSubcarrierInfo_pt->HLOGpsus[1];
				for (index=1;index<64;index++)
				{
					pts.adslATURSubcarrierInfo_pt->HLOGpsus[index*2]   = (pts.adslATURSubcarrierInfo_pt->HLOGpsus[(index)*2-1] +  pts.adslATURSubcarrierInfo_pt->HLOGpsus[(index)*2+1]) >>1;
				}
			}
#endif
		}
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_QLNMT)){
			NEAREND_QLNMT_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group INFO Address 80 Index 1");
#endif
				pts.adslATURSubcarrierInfo_pt->QLNMTus = 0;	
			}
			else{
				pts.adslATURSubcarrierInfo_pt->QLNMTus = RxMessage[4];					
			}
		}
		
		/////////////////////////////////////////////////////////////////////////
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_QLNPS)){
			int index=0,size=12;
			...
			for (index=0;index<32;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
						...
						if (adsl_mode < 0x4000 && adsl_mode_extend==0)//adsl2 mode
						{
							pts.adslATURSubcarrierInfo_pt->QLNpsus[(index+msg_idx)*4+1] = (u16)(RxMessage[4+msg_idx]&0xFF);
							pts.adslATURSubcarrierInfo_pt->QLNpsus[(index+msg_idx)*4+3] = (u16)((RxMessage[4+msg_idx]>>8)&0xFF);
						}else
#endif
						{
							pts.adslATURSubcarrierInfo_pt->QLNpsus[(index+msg_idx)*2] = (u16)(RxMessage[4+msg_idx]&0xFF);
							pts.adslATURSubcarrierInfo_pt->QLNpsus[(index+msg_idx)*2+1] = (u16)((RxMessage[4+msg_idx]>>8)&0xFF);
						//printk("index:%d ,cmv_result: %04X\n",index+msg_idx,RxMessage[4+msg_idx]);
						}
					}
			...
			if (adsl_mode == 0x200 || adsl_mode == 0x800 || adsl_mode ==0x2000  || adsl_mode ==0x4000 || (adsl_mode == 0 && (adsl_mode_extend == 0x4 || adsl_mode_extend == 0x2))//ADSL 2 Annex B(0x200)/J(0x800)/M(0x2000) //ADSL 2+ B,J,M
			{
				pts.adslATURSubcarrierInfo_pt->QLNpsus[0] = pts.adslATURSubcarrierInfo_pt->QLNpsus[1];
				for (index=1;index<64;index++)
				{
					pts.adslATURSubcarrierInfo_pt->QLNpsus[index*2]   = (pts.adslATURSubcarrierInfo_pt->QLNpsus[(index)*2-1] +  pts.adslATURSubcarrierInfo_pt->QLNpsus[(index)*2+1]) >>1;
				}
			}
#endif
		}
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_SNRMT)){
			NEAREND_SNRMT_MAKECMV(H2D_CMV_READ);
			if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
#ifdef DANUBE_MEI_DEBUG_ON
				printk("\n\nCMV fail, Group INFO Address 80 Index 2");
#endif
				pts.adslATURSubcarrierInfo_pt->SNRMTus = 0;	
			}
			else{
				pts.adslATURSubcarrierInfo_pt->SNRMTus = RxMessage[4];					
			}
		}
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_SNRPS)){
			int index=0,size=12;
			...
			for (index=0;index<64;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					//memcpy(&pts.adslATURSubcarrierInfo_pt->SNRpsus[index],&RxMessage[4],size*2);
					int msg_idx=0;
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
						pts.adslATURSubcarrierInfo_pt->SNRpsus[index+msg_idx] = (u16)(RxMessage[4+msg_idx]&0xFF);
						//printk("index:%d ,cmv_result: %04X\n",index+msg_idx,RxMessage[4+msg_idx]);
					}
		...
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_BITPS)){
			int index=0,size=12;
			...
			for (index=0;index<32;index+=size)
			{
				...
				if(meiCMV(TxMessage, YES_REPLY, RxMessage)!=MEI_SUCCESS){
					...
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
						pts.adslATURSubcarrierInfo_pt->BITpsus[(index+msg_idx)*2] = (u16)(RxMessage[4+msg_idx]&0xFF);
						pts.adslATURSubcarrierInfo_pt->BITpsus[(index+msg_idx)*2+1] = (u16)((RxMessage[4+msg_idx]>>8)&0xFF);
						//printk("index:%d ,cmv_result: %04X\n",index+msg_idx,RxMessage[4+msg_idx]);
					}
		...
		if(IS_FLAG_SET((&(pts.adslATURSubcarrierInfo_pt->flags)),  NEAREND_GAINPS)){
			int index=0,size=12;
			for (index=0;index<64;index+=size)
			{
				if (index+size>=64)
				...
				/*
					...
					*/
					memcpy(&pts.adslATURSubcarrierInfo_pt->GAINpsus[index],&RxMessage[4],size*2);	
#if 0
					int msg_idx;
					for (msg_idx=0;msg_idx<size;msg_idx++)
					{
		...
		copy_to_user((char *)lon, (char *)pts.adslATURSubcarrierInfo_pt, sizeof(adslATUCSubcarrierInfo));		
...
void danube_mei_mib_cleanup(void)
{
	...
	wake_up(&wait_queue_mibdaemon); //wake up mibdaemon
