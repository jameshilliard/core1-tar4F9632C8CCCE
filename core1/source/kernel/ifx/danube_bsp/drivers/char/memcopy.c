...
/**
 * @file description:
 *	Driver for Infineon Danube memcopy
 */
//-----------------------------------------------------------------------
/*
 * \author:	Wu Qi Ming[Qi-Ming.Wu@infineon.com]
 * Created:	11-Augest-2005
 */
 ...
 * \ingroup Internal
 */
int memcopy_receive(_danube_memcopy_device* pDev)
{
   int len;
   u8* buf;
 ...
 * \ingroup Internal
 */
int transmit_data(_danube_memcopy_device* pDev)
{
    int len;
    ...
    while(pDev->src_count<pDev->len)
    {
      ...
      dma_device_write(dma_dev,pDev->src+pDev->src_count,len,NULL);
      pDev->src_count+=len;
    }
 ...
 * \ingroup API
 */
int danube_dma_memcopy(_danube_memcopy_device* pDev,u8* dst,u8* src,int len)
{
        int i;
	int ret=0;
	...
	pDev->dst_count=0;
	pDev->recv_count=0;
	if(len>MAX_BLOCK_SIZE) pDev->packet_size=MAX_BLOCK_SIZE;
	else pDev->packet_size=len;
...
#endif
	if(pDev->recv_count==pDev->len) ret=1;
	else
	{
 ...
 * \ingroup API
 */
static int memcopy_open(struct inode * inode, struct file * file)
{
        int result=0;
	int num;
        struct dma_device_info *dma_device;
...
static int memcopy_release(struct inode *inode, struct file *file)
{
        int num = MINOR(inode->i_rdev);
	struct dma_device_info *dma_device;
        dma_device=((_danube_memcopy_device*)file->private_data)->dma_device;
	if(dma_device) dma_device_release(dma_device);
	MOD_DEC_USE_COUNT;
        return 0;
}

static struct page* dma_mmap_nopage(struct vm_area_struct *area,
...
static ssize_t memcopy_write(struct file *file,const char *buf, size_t count, loff_t *offset)
{
        return count;
}

static ssize_t memcopy_read(struct file *file,char *buf, size_t count, loff_t *offset)
{
        return 0;
}
 ...
 * \ingroup API
 */
static int memcopy_ioctl(struct inode *inode, struct file *file,
             unsigned int cmd, unsigned long arg)
{
     ...
     struct memcopy_req* req=(struct memcopy_req*)kmalloc(sizeof(struct memcopy_req),GFP_KERNEL);
     switch(cmd)
     {
       case MEMCOPY:
       memset((u8*)g_src,0,4096);
       memset((u8*)g_dst,0,4096);
       copy_from_user(req,(struct memcopy_req*)arg,sizeof(struct memcopy_req));
