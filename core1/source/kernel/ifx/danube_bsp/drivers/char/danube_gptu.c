 ...
 *
 *  Description:
 *    device driver of general purpose timer/clock unit (GPTU) of Danube chip
 *  Author:
 *    Samuels Xu Liang
 *  Created:
 *    26 Jul 2005
 ...
 *    X is either A or B
 */
#define GPTU_RELOAD_VALUE(n, X)         (X == A && GPTU_CON_EXT(n, X) ? *DANUBE_GPTU_RELOAD(n, X) : *DANUBE_GPTU_RELOAD(n, X) & 0xFFFF)
...
int set_counter(unsigned int timer, int is_32bit, int is_sync, int is_ext_src, unsigned int trigger_flag, unsigned int handle_flag, unsigned long arg1, unsigned long arg2)
{
    ...
    if ( timer == TIMER1B )
    {
        flag = (is_sync ? TIMER_FLAG_SYNC : TIMER_FLAG_UNSYNC)
               ...
               | TIMER_FLAG_CYCLIC | TIMER_FLAG_COUNTER | TIMER_FLAG_DOWN | TIMER_FLAG_NO_HANDLE
    ...
    flag = (is_sync ? TIMER_FLAG_SYNC : TIMER_FLAG_UNSYNC)
           | (is_32bit ? TIMER_FLAG_32BIT : TIMER_FLAG_16BIT)
           | TIMER_FLAG_ONCE | TIMER_FLAG_COUNTER | TIMER_FLAG_UP | TIMER_FLAG_NO_HANDLE
               | TIMER_FLAG_MASK_TRIGGER(trigger_flag)
               | TIMER_FLAG_MASK_HANDLE(handle_flag);
