...
/* 21July2005   jssow   Generate from AMAZON project */
...
#include <asm-mips/danube/danube_gpio.h>
//#include <asm-mips/danube/port.h>
//#include <asm-mips/danube/danube_gpio_version.h>
#include <asm/danube/danube.h>
#include "port.h"

const char GPIO_VERSION[] = "GPIO_VERSION_0.0.1";
					...
					unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_DIR(cmd) & _IOC_READ) {
        ...
        if(_IOC_NR(cmd) > DANUBE_PORT_IOC_MAXNR) return -ENOTTY;
...
/**     Brief: gpio driver version proc entry read
  *
  *	Return Value :
  *            len   : OK
  */

int danube_gpio_getversionproc(char *buf)
{
	int len;
        len = sprintf(buf, "%s\n" , &GPIO_VERSION[21]);
        return len;
...
int __init danube_port_init(void)
{
	...
	pins = 32; //danube_mux_gpio_pins(t);
	...
	/* Create proc file */
	create_proc_read_entry("danube_port", 0, NULL, danube_port_read_procmem, NULL);
        create_proc_read_entry("danube_gpio_version", 0, NULL, danube_port_read_procmem, (void *)danube_gpio_getversionproc);
	return OK;
}
