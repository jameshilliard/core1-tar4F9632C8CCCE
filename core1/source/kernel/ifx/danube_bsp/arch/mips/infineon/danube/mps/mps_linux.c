...
s32 ifx_mps_register_data_callback(mps_devices type, u32 dir, void (*callback)(mps_devices type));
s32 ifx_mps_unregister_data_callback(mps_devices type, u32 dir);
s32 ifx_mps_register_event_callback(mps_devices type, MbxEventRegs_s* mask, void (*callback)(MbxEventRegs_s* events));
 ...
 * \ingroup Internal
 */
void ifx_mbx_dummy_rcv_data_callback( mps_devices type )
{
	mps_message rw;
    ...
    switch (rw.cmd_type)
    {
        case CMD_RTP_VOICE_DATA_PACKET:
            ...
            if (ifx_mps_rtp_voice_data_count % 10000 == 0) {
                printk("%s - %d packets received\n", __FUNCTION__,ifx_mps_rtp_voice_data_count);
            }
            ...
            else kfree((char*)KSEG0ADDR(rw.pData));                
            break;
                  ...
                  u32 nCmd, unsigned long arg)
{
    ...
    case FIO_MPS_MB_WRITE:
        ...
        else
        {
            ...
            rw_struct.pData = ifx_mps_bufman_malloc(rw_struct.nDataBytes, GFP_KERNEL);
            ...
            {
                retvalue = ifx_mps_mbx_write_data(pMBDev, &rw_struct);
            }
            /* ... and finally restore the buffer pointer and copy mps_message back! */
            rw_struct.pData = (u8*)pUserBuf;
            copy_to_user((char*)arg, (char*)&rw_struct, sizeof(mps_message));
...
s32 ifx_mps_register_event_callback(mps_devices type, MbxEventRegs_s* mask, void (*callback)(MbxEventRegs_s* events))
{
    ...
    {
        memcpy((char*)&pMBDev->event_mask, (char*)mask, sizeof(MbxEventRegs_s));
...
s32 ifx_mps_event_activation(mps_devices type, MbxEventRegs_s* act)
{
    ...
    *DANUBE_MPS_AD1ENR = Ad1Reg.val;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        VCStatReg.val = (VCStatReg.val & ~pMBDev->event_mask.MPS_VCStatReg[i].val) 
                   | (act->MPS_VCStatReg[i].val & pMBDev->event_mask.MPS_VCStatReg[i].val);
 ...
 * \ingroup Internal  
 */
static int ifx_mps_read_proc(char *page, char **start, off_t off,
 ...
 * \ingroup Internal  
 */
static int ifx_mps_write_fw_procmem(struct file *file, const char *buffer,
                    unsigned long count, void *data)
{
    ...
    if (count==(sizeof(MPS_FW_START_TAG))) {
        if (!strncmp(buffer,MPS_FW_START_TAG,sizeof(MPS_FW_START_TAG)-1)) {
            ...
            ifx_mps_firmware_struct.data = (u32*) ifx_mps_firmware_buffer;
            ifx_mps_firmware_struct.length = ifx_mps_firmware_buffer_pos;
            ifx_mps_download_firmware(NULL, &ifx_mps_firmware_struct);
            return count;
        }
    }
    ...
    if (count==(sizeof(MPS_FW_OPEN_VOICE_TAG))) {
        if (!strncmp(buffer,MPS_FW_OPEN_VOICE_TAG,sizeof(MPS_FW_OPEN_VOICE_TAG)-1)) {
    ...
    if (count==(sizeof(MPS_FW_REGISTER_CALLBACK_TAG))) {
        if (!strncmp(buffer,MPS_FW_REGISTER_CALLBACK_TAG,sizeof(MPS_FW_REGISTER_CALLBACK_TAG)-1)) {
            printk("Opening voice0 mailbox...\n");
    ...
    if (count==(sizeof(MPS_FW_SEND_MESSAGE_TAG))) {
        if (!strncmp(buffer,MPS_FW_SEND_MESSAGE_TAG,sizeof(MPS_FW_SEND_MESSAGE_TAG)-1)) {
            ...
            for(i = 0; i < (100 / 4); i++)
                *((u32*) KSEG1ADDR((wrt_data.pData+(i<<2)))) = 0xdeadbeef;
    ...
    if (count==(sizeof(MPS_FW_RESTART_TAG))) {
        if (!strncmp(buffer,MPS_FW_RESTART_TAG,sizeof(MPS_FW_RESTART_TAG)-1)) {
    ...
    if (count==(sizeof(MPS_FW_ENABLE_PACKET_LOOP_TAG))) {
        if (!strncmp(buffer,MPS_FW_ENABLE_PACKET_LOOP_TAG,sizeof(MPS_FW_ENABLE_PACKET_LOOP_TAG)-1)) {
    ...
    if (count==(sizeof(MPS_FW_DISABLE_PACKET_LOOP_TAG))) {
        if (!strncmp(buffer,MPS_FW_DISABLE_PACKET_LOOP_TAG,sizeof(MPS_FW_DISABLE_PACKET_LOOP_TAG)-1)) {
...
#endif
{
    ...
    devfs_register(mps_dir, "voice0", DEVFS_FL_NONE, ifx_mps_major_id, 2, S_IFCHR | S_IRUGO | S_IWUGO, &ifx_mps_fops, NULL);
    devfs_register(mps_dir, "voice1", DEVFS_FL_NONE, ifx_mps_major_id, 3, S_IFCHR | S_IRUGO | S_IWUGO, &ifx_mps_fops, NULL);
    devfs_register(mps_dir, "voice2", DEVFS_FL_NONE, ifx_mps_major_id, 4, S_IFCHR | S_IRUGO | S_IWUGO, &ifx_mps_fops, NULL);
    devfs_register(mps_dir, "voice3", DEVFS_FL_NONE, ifx_mps_major_id, 5, S_IFCHR | S_IRUGO | S_IWUGO, &ifx_mps_fops, NULL);
    ...
    result = request_irq(/*INT_NUM_IM5_IRL11*/INT_NUM_IM4_IRL18, ifx_mps_ad0_irq,
                         SA_INTERRUPT, "mps_mbx ad0", &ifx_mps_dev);
    if (result)
        return result;

    result = request_irq(/*INT_NUM_IM5_IRL12*/INT_NUM_IM4_IRL19, ifx_mps_ad1_irq,
    ...
    for (i = 0; i < 4; ++i)
    {
        sprintf(&voice_channel_int_name[i][0],"mps_mbx vc%d",i);
            result = request_irq(/*INT_NUM_IM5_IRL7*/INT_NUM_IM4_IRL14 + i, ifx_mps_vc_irq,
			      SA_INTERRUPT, &voice_channel_int_name[i][0], &ifx_mps_dev);
	    if (result)
	        return result;
    }
    ...
    if ( ifx_mps_proc_dir != NULL )
    {
        ...
        create_proc_read_entry("version" , S_IFREG|S_IRUGO,
                               ifx_mps_proc_dir, ifx_mps_read_proc, (void *)ifx_mps_get_version_proc);
        create_proc_read_entry("status" , S_IFREG|S_IRUGO,
                               ifx_mps_proc_dir, ifx_mps_read_proc, (void *)ifx_mps_get_status_proc);
    }
    else
    {
