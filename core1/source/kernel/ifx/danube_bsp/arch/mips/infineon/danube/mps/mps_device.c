...
#undef USE_ENCRYPTED_VOICE_FIRMWARE
...
static s32 ifx_mps_mbx_write_buffer_prov_message(mem_seg_t *mem_ptr, u8 segments, u32 segment_size );
...
void* ifx_mps_fastbuf_malloc(size_t size, int priority)
{
	...
	do
	{
		if (index == FASTBUF_BUFS) index = 0;
		if (fastbuf_pool[index] & FASTBUF_USED)
			continue;
		ptr = fastbuf_pool[index];
		fastbuf_pool[index] |= FASTBUF_USED;
		fastbuf_index = index;
		restore_flags(flags);
		return (void*) ptr;
	}
...
void ifx_mps_fastbuf_free(const void* ptr)
{
	...
	do
	{
		if (index < 0) index = FASTBUF_BUFS - 1;
		if (fastbuf_pool[index] == ((u32)ptr | FASTBUF_USED))
		{
			fastbuf_pool[index] &= ~FASTBUF_USED;
			restore_flags(flags);
			return;
...
s32 ifx_mps_fastbuf_init(void)
{
	...
	if ((fastbuf_ptr = kmalloc((FASTBUF_BUFS * bufsize) , GFP_KERNEL)) == NULL)
		return -ENOMEM;
	ptr = fastbuf_ptr;
 ...
 * \ingroup Internal
 */
static s32 ifx_mps_bufman_get_level(void)
{
...
static mps_buffer_state_e ifx_mps_bufman_update_state(void)
{
    if(mps_buffer.buf_state != MPS_BUF_ERR)
    {
        ...
        if( (mps_buffer.buf_level > 0) &&  (mps_buffer.buf_level < mps_buffer.buf_threshold) )
            mps_buffer.buf_state = MPS_BUF_LOW;
        if( (mps_buffer.buf_level >= mps_buffer.buf_threshold) &&  (mps_buffer.buf_level <= MPS_BUFFER_MAX_LEVEL) )
 ...
 * \ingroup Internal
 */
static s32 ifx_mps_bufman_inc_level(u32 value)
{
    u32 flags;
 ...
 * \ingroup Internal
 */
static s32 ifx_mps_bufman_dec_level(u32 value)
{
    u32 flags;
 ...
 * \ingroup Internal
 */
static mps_buffer_state_e ifx_mps_bufman_get_state(void)
{
    return mps_buffer.buf_state;
}
...
s32 ifx_mps_bufman_init(void)
{
	...
	for(i = 0; i < mps_buffer.buf_initial; i+= MPS_DEFAULT_PROVISION_SEGMENTS_PER_MSG)
    {
    	ret = ifx_mps_bufman_buf_provide(MPS_DEFAULT_PROVISION_SEGMENTS_PER_MSG,
                                         mps_buffer.buf_size);
    }
    return ret;
}
 ...
 */
void ifx_mps_bufman_register(void* (*malloc)(size_t size, int priority), void (*free)(const void* ptr), u32 buf_size, u32 treshold)
{
...
static s32 ifx_mps_bufman_buf_provide(u32 segments, u32 segment_size)
{
    ...
    {
        mem_seg_ptr[i] = (u32*) PHYSADDR((u32)mps_buffer.malloc(segment_size, GFP_ATOMIC));
    ...
    if (ifx_mps_mbx_write_buffer_prov_message(&mem_seg_ptr, segments, segment_size) != OK)
    {
        goto error;
    }
    else
...
error:  
    for(j=i-1; j>=0; j--)
        mps_buffer.free(KSEG0ADDR(mem_seg_ptr[j]));
    return -1;
}
 ...
 * \ingroup Internal
 */
void ifx_mps_fifo_clear(mps_fifo *fifo)
{
 ...
 * \ingroup Internal
 */
bool_t ifx_mps_fifo_not_empty(mps_fifo *fifo)
{
 ...
 * \ingroup Internal
 */
bool_t ifx_mps_fifo_mem_request(mps_fifo *fifo, u32 bytes)
{
 ...
 * \ingroup Internal 
 */
void ifx_mps_fifo_read_ptr_inc(mps_fifo *fifo, u8 increment)
{
 ...
 * \ingroup Internal 
 */
void ifx_mps_fifo_write_ptr_inc(mps_fifo *fifo, u16 increment)
{
    ...
    {
        *fifo->pwrite_off = (u32)(new_write_index + (s32)(fifo->size));
    }
    return;
}
 ...
 * \ingroup Internal 
 */
s32 ifx_mps_fifo_write(mps_fifo *fifo, u32 data, u8 offset)
{
 ...
 * \ingroup Internal 
 */
u32 ifx_mps_fifo_read(mps_fifo *fifo, u8 offset)
{
    u32 read_address;
    ...
    read_address =  (u32)fifo->pend + (u32)*fifo->pread_off - (u32)offset;
    if( new_read_index < 0 )
    {
...
//    *DANUBE_BIU_FB1_CFG = 0x00000001;
    ...
    *DANUBE_GPTC_CON_1B    = 0x000005C1; // 0x000001C4;
...
#if 0
      set_timer(TIMER1B, 8000000, 1, TIMER_FLAG_CALLBACK_IN_IRQ, NULL, NULL);
      start_timer(TIMER1B, 0);
      disable_irq(INT_NUM_IM3_IRL23);
#endif

#if 1
      set_counter(TIMER1B, 0, 1, 0, TIMER_FLAG_ANY_EDGE, TIMER_FLAG_CALLBACK_IN_IRQ, NULL, NULL);
      start_timer(TIMER1B, 0);
//      disable_irq(INT_NUM_IM3_IRL23);
#endif
 ...
 * \ingroup Internal
 */
s32 ifx_mps_download_firmware(mps_mbx_dev *pMBDev, mps_fw *pFWDwnld)
{
#if 0
    u32 reset_ctrl;
#endif
    ...
    memcpy((char*)danube_cp1_base,(char*)pFWDwnld->data,pFWDwnld->length);
 ...
 * \ingroup Internal
 */
s32 ifx_mps_common_open(mps_comm_dev *mps_device, mps_mbx_dev *pMBDev, bool_t bcommand)
{
    MPS_Ad0Reg_u Ad0Reg;
...
u32 ifx_mps_init_structures(mps_comm_dev *pDev)
{
    ...
    * set command mailbox sub structure pointers
     * to global mailbox register addresses
    */
    pDev->command_mb.upstrm_fifo.pstart =
        (u32*)KSEG1ADDR((MBX_Memory->MBX_UPSTR_CMD_BASE + ((MBX_Memory->MBX_UPSTR_CMD_SIZE - 4)>>2)));
    pDev->command_mb.upstrm_fifo.pend       = (u32*)KSEG1ADDR(MBX_Memory->MBX_UPSTR_CMD_BASE);
    pDev->command_mb.upstrm_fifo.pwrite_off = (u32*)&(MBX_Memory->MBX_UPSTR_CMD_WRITE);
    pDev->command_mb.upstrm_fifo.pread_off  = (u32*)&(MBX_Memory->MBX_UPSTR_CMD_READ);
    pDev->command_mb.upstrm_fifo.size       = MBX_Memory->MBX_UPSTR_CMD_SIZE;
#ifdef CONFIG_PROC_FS
    pDev->command_mb.upstrm_fifo.min_space = MBX_Memory->MBX_UPSTR_CMD_SIZE;
#endif


    pDev->command_mb.dwstrm_fifo.pstart =
        (u32*)KSEG1ADDR((MBX_Memory->MBX_DNSTR_CMD_BASE + ((MBX_Memory->MBX_DNSTR_CMD_SIZE - 4)>>2)));
    pDev->command_mb.dwstrm_fifo.pend       = (u32*)KSEG1ADDR(MBX_Memory->MBX_DNSTR_CMD_BASE);
    pDev->command_mb.dwstrm_fifo.pwrite_off = (u32*)&(MBX_Memory->MBX_DNSTR_CMD_WRITE);
    pDev->command_mb.dwstrm_fifo.pread_off  = (u32*)&(MBX_Memory->MBX_DNSTR_CMD_READ);
    pDev->command_mb.dwstrm_fifo.size       = MBX_Memory->MBX_DNSTR_CMD_SIZE;
#ifdef CONFIG_PROC_FS
    pDev->command_mb.dwstrm_fifo.min_space = MBX_Memory->MBX_DNSTR_CMD_SIZE;
#endif
    ...
    pDev->command_mb.sem_dev = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
    ...
    for ( i=0; i<NUM_VOICE_CHANNEL; i++ )
    {
        ...
        pDev->voice_mb[i].upstrm_fifo.pstart      = (u32*)KSEG1ADDR(MBX_Memory->MBX_UPSTR_DATA_BASE +
                ((MBX_Memory->MBX_UPSTR_DATA_SIZE - 4)>>2));
        pDev->voice_mb[i].upstrm_fifo.pend        = (u32*)KSEG1ADDR(MBX_Memory->MBX_UPSTR_DATA_BASE);
        pDev->voice_mb[i].upstrm_fifo.pwrite_off  = (u32*)&(MBX_Memory->MBX_UPSTR_DATA_WRITE);
        pDev->voice_mb[i].upstrm_fifo.pread_off   = (u32*)&(MBX_Memory->MBX_UPSTR_DATA_READ);
        pDev->voice_mb[i].upstrm_fifo.size        = MBX_Memory->MBX_UPSTR_DATA_SIZE;
#ifdef CONFIG_PROC_FS
        pDev->voice_mb[i].upstrm_fifo.min_space         = MBX_Memory->MBX_UPSTR_DATA_SIZE;
#endif

        /* voice downstream data mailbox area */
        pDev->voice_mb[i].dwstrm_fifo.pstart      = (u32*)KSEG1ADDR(MBX_Memory->MBX_DNSTR_DATA_BASE +
                ((MBX_Memory->MBX_DNSTR_DATA_SIZE - 4)>>2));
        pDev->voice_mb[i].dwstrm_fifo.pend        = (u32*)KSEG1ADDR(MBX_Memory->MBX_DNSTR_DATA_BASE);
        pDev->voice_mb[i].dwstrm_fifo.pwrite_off  = (u32*)&(MBX_Memory->MBX_DNSTR_DATA_WRITE);
        pDev->voice_mb[i].dwstrm_fifo.pread_off   = (u32*)&(MBX_Memory->MBX_DNSTR_DATA_READ);
        pDev->voice_mb[i].dwstrm_fifo.size        = MBX_Memory->MBX_DNSTR_DATA_SIZE;
#ifdef CONFIG_PROC_FS
    	pDev->voice_mb[i].dwstrm_fifo.min_space         = MBX_Memory->MBX_UPSTR_DATA_SIZE;
#endif
        ...
        /* initialize the semaphores for multitasking access */
        pDev->voice_mb[i].sem_dev = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
        sema_init(pDev->voice_mb[i].sem_dev, 1);

        /* initialize the semaphores to read from the fifo */
        pDev->voice_mb[i].sem_read_fifo = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
        sema_init(pDev->voice_mb[i].sem_read_fifo, 0);

#ifdef MPS_FIFO_BLOCKING_WRITE
        pDev->voice_mb[i].sem_write_fifo = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
        ...
        pDev->voice_mb[i].wakeup_pending = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
    ...
    pDev->provide_buffer = kmalloc(sizeof(struct semaphore), GFP_KERNEL);
...
u32 ifx_mps_reset_structures(mps_comm_dev *pDev)
{
    ...
    for (i = 0; i < NUM_VOICE_CHANNEL; i++)
    {
        ...
        pDev->voice_mb[i].upstrm_fifo.min_space = pDev->voice_mb[i].upstrm_fifo.size;
        pDev->voice_mb[i].dwstrm_fifo.min_space = pDev->voice_mb[i].dwstrm_fifo.size;
#endif           
    }
 ...
 * \ingroup Internal 
 */
s32 ifx_mps_mbx_get_message_length(mps_fifo *mbx )
{
    MbxMsgHd_u  msg_hd;
 ...
 * \ingroup Internal 
 */
s32 ifx_mps_mbx_read_message(mps_fifo *fifo, MbxMsg_s *msg, u32 *bytes)
{
    s32 i;
    u32 flags;
    ...
    {
        printk("%s(): Odd payload length %d\n", __FUNCTION__, msg->header.hd.plength);
        return -1;
    }
...
s32 ifx_mps_mbx_read(mps_mbx_dev *pMBDev, mps_message *pPkg, s32 timeout)
{
    ...
    if(ifx_mps_mbx_read_message(fifo, &msg, &bytes) == 0)
    {
        ...
        case voice3:
             ...
             */
            pPkg->cmd_type       = msg.header.hd.type;
            pPkg->pData          = KSEG0ADDR((u8*)msg.data[0]); /* get payload pointer */
            pPkg->nDataBytes     = msg.data[1];                 /* get payload size */

#ifndef CONFIG_MIPS_UNCACHED
            dma_cache_inv(KSEG0ADDR(pPkg->pData), pPkg->nDataBytes);
#endif            
			...
			*/ 
            ...
            {
                if (ifx_mps_bufman_buf_provide(MPS_DEFAULT_PROVISION_SEGMENTS_PER_MSG, mps_buffer.buf_size) != OK)
                {
 ...
 * \ingroup Internal
 */
u32 ifx_mps_mbx_build_word(u8* byte_ptr)
{
 ...
 * \ingroup Internal
 */
s32 ifx_mps_mbx_write_message(mps_mbx_dev *pMBDev, u8 *msg_ptr, u32 msg_bytes)
{
    mps_fifo *mbx;
    u32     i;
    u32     flags;
    ...
    */
    ...
    if(retries < MAX_FIFO_WRITE_RETRIES)
    {
        ...
        */
		...
		{
		   	if( word_aligned )
        		ifx_mps_fifo_write( mbx, *(u32*)(msg_ptr+i), i);
          	else
          	{
...
#endif
        if ((retval = ifx_mps_mbx_write_message(pMBDev, (u8*)&msg, 12)) != OK)
        {
 ...
 * \ingroup Internal
 */
s32 ifx_mps_mbx_write_buffer_prov_message(mem_seg_t *mem_ptr, u8 segments, 
                                          u32 segment_size )
{
    int retval = ERROR;
    int i;
     ...
     */
    retval = ifx_mps_mbx_write_message((&pMPSDev->voice_mb[0]), (u8*)&msg, (u32)(segments+2)*4 );
    if(retval == OK)
    {
...
s32 ifx_mps_mbx_write_cmd(mps_mbx_dev *pMBDev, mps_message *readWrite)
{
    ...
    if( pMBDev->devID == command )
    {
        ...
        retval=ifx_mps_mbx_write_message( pMBDev, (u8*)readWrite->pData, readWrite->nDataBytes);
    }
    else
    {
...
notify_udq_again:
    ...
    if( ifx_mps_fifo_not_empty( mbx ) )
    {
        ...
        /* read message header from buffer */
        msg.header.val = ifx_mps_fifo_read(&pMPSDev->voice_mb[0].upstrm_fifo, 0);
        if( msg.header.hd.type == CMD_ADDRESS_PACKET ) 
        {
            int i;

            ifx_mps_mbx_read_message(&pMPSDev->voice_mb[0].upstrm_fifo, &msg, &bytes_read);
            for (i = 0; i < (msg.header.hd.plength/4 - 1); i++)
            {
                mps_buffer.free((void*)KSEG0ADDR(msg.data[i]));
            }
            restore_flags(flags);
            goto notify_udq_again;
        }
        else
        {
#ifdef CONFIG_PROC_FS
            ...
            if((ifx_mps_bufman_get_level() <= mps_buffer.buf_threshold) 
	           && (sem_getcount(pMPSDev->provide_buffer) == 0))
	        {
	        	up(pMPSDev->provide_buffer);
	        }
            ...
            else
            {
	            ...
	            if (down_trylock(pMPSDev->wakeup_pending) == 0)
	            {
...
static void ifx_mps_mbx_cmd_upstream(unsigned long dummy)
{
    ...
    if ( ifx_mps_fifo_not_empty( mbx ) )
    {
        ...
        else
        {
            ...
            wake_up_interruptible(&(pMPSDev->command_mb.mps_wakeuplist));
        }
    }
    return;
}
 ...
 * \ingroup Internal
 */
void ifx_mps_ad0_irq(int irq, mps_comm_dev *pDev, struct pt_regs *regs)
{
 ...
 * \ingroup Internal
 */
void ifx_mps_ad1_irq(int irq, mps_comm_dev *pDev, struct pt_regs *regs)
{
    ...
    *DANUBE_MPS_CAD1SR = MPS_Ad1StatusReg.val;  /* and acknowledge */
    ...
    pMPSDev->event.MPS_Ad1Reg.val = MPS_Ad1StatusReg.val & mbx_dev->event_mask.MPS_Ad1Reg.val;
 ...
 * \ingroup Internal
 */
void ifx_mps_vc_irq(int irq, mps_comm_dev *pDev, struct pt_regs *regs)
{
    ...
    pMPSDev->event.MPS_VCStatReg[chan].val = MPS_VCStatusReg.val & mbx_dev->event_mask.MPS_VCStatReg[chan].val;
...
int ifx_mps_print_fw_version(void)
{
    ...
    /* send message */
    retval=ifx_mps_mbx_write_message( &(ifx_mps_dev.command_mb), (u8*)&msg, 4);
