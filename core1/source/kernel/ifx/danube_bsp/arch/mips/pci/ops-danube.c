...
static int danube_pci_config_access(unsigned char access_type,
	struct pci_dev *dev, unsigned char where, u32 *data)
{
  ...
  if ( (bus !=0) || ((dev_fn&0xf8)>0x78) || ((dev_fn&0xf8) == 0) || ((dev_fn&0xf8) == 0x68) ) {
    return 1;
  }

  DBP("\nbus=%x dev_fn=%x %s",bus,dev_fn,(access_type==PCI_ACCESS_WRITE)?"WRITE":"READ");
  pci_addr=PCI_CFG_BASE|
    bus<<DANUBE_PCI_CFG_BUSNUM_SHF|
    dev_fn<<DANUBE_PCI_CFG_FUNNUM_SHF|
    (where & ~0x3);
 ...
 */
static int danube_pci_read_config_byte (struct pci_dev *dev, int where,
	u8 *val)
{
	u32 data = 0;
...
static int danube_pci_read_config_word (struct pci_dev *dev, int where,
	u16 *val)
{
	u32 data = 0;
...
static int danube_pci_read_config_dword (struct pci_dev *dev, int where,
	u32 *val)
{
	u32 data = 0;
...
static int danube_pci_write_config_byte (struct pci_dev *dev, int where,
	u8 val)
{
	u32 data = 0;
...
static int danube_pci_write_config_word (struct pci_dev *dev, int where,
	u16 val)
{
        u32 data = 0;
...
static int danube_pci_write_config_dword(struct pci_dev *dev, int where,
	u32 val)
{
	if (where & 3)
		return PCIBIOS_BAD_REGISTER_NUMBER;
...
//prochao+, 10/17/2006, for supporting the WiFi on the CPU1 core with another interrupt number
            ...
            dev->irq = INT_NUM_IM0_IRL26;   //on CPU1
#else
			dev->irq = INT_NUM_IM0_IRL22;   //on CPU0
#endif
